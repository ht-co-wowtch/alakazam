...
func load() *Config {
	...
	return &Config{
		...
		RPCClient: &RPCClient{
			Dial: xtime.Duration(time.Duration(viper.GetInt("rpcClient.timeout")) * time.Second),
		},
		RPCServer: &RPCServer{
			Network:           "tcp",
		...
		Protocol: &Protocol{
			...
			HandshakeTimeout: xtime.Duration(time.Second * time.Duration(viper.GetInt("protocol.handshakeTimeout"))),
		},
		Bucket: &Bucket{
