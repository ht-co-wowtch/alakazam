...
func TestSetUser(t *testing.T) {
	...
	err := d.SetUser(uid, key, roomId, name, "", business.PlayDefaultPermission)
	assert.Nil(t, err)
}
...
func mockSetUser(uid string, key string, roomId string, name string) {
	mock.Command("HSET", keyUidInfo(uid), key, roomId, hashNameKey, name, hashStatusKey, business.PlayDefaultPermission, hashServerKey, "").
		Expect("")
