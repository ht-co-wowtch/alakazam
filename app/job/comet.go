...
func NewComet(c *conf.Comet) (*Comet, error) {
	cmt := &Comet{
		...
		pushChan:      make(chan *pb.KeyReq, 100),
...
// 處理訊息推送給comet server
func (c *Comet) process(roomChan chan *pb.BroadcastRoomReq, broadcastChan chan *pb.BroadcastReq, pushChan chan *pb.KeyReq) {
	for {
		select {
		// 多個房間推送
		...
		case keysArg := <-pushChan:
			_, err := c.client.Push(context.Background(), keysArg)
			if err != nil {
				log.Error("grpc client push key", zap.Error(err), zap.Any("arg", keysArg))
			}
		case <-c.ctx.Done():
			return
		}
